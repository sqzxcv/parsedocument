 ...
 * @author 王德封
 */
public class LLCollectorManager {
     ...
     */
    public static void addJob(String jobName, Job jobClass, String time) {
        try {
            Scheduler sched = gSchedulerFactory.getScheduler();
            JobDetail jobDetail = JobBuilder.newJob(LLQuartzJob.class)
                              .withIdentity(jobName)
                    .build();
//            JobDetail jobDetail = new JobDetail(, JOB_GROUP_NAME, Class.forName(jobClass));// 任务名，任务组，任务执行类
            // 触发器
            //3、获得一个trigger
